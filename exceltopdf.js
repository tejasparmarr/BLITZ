// Backend-integrated Excel → PDF converter
...
function estimateSheetsFromSize(bytes){ if(!bytes) return '—'; const avg=300*1024; return Math.max(1, Math.round(bytes/avg)); }
...
function resetUI(){
  setStatus('Select an Excel file to begin.'); setProgress(0);
  $('#kFileName').textContent='—'; $('#kFileSize').textContent='—'; $('#kSheets').textContent='—';
  $('#downloadBloc').classList.add('hidden'); $('#convertBtn').disabled=true;
}

// Store file globally for drag-drop
...
function onFileChosen(file){
  ...
  $('#kSheets').textContent=estimateSheetsFromSize(file.size);
...
function bindDropzone(){
  ...
  input?.addEventListener('change', e=>{ const f=e.target.files?.[0]; if(f) onFileChosen(f); });
...
async function convertExcelToPdf(file) {
  ...
  try {
    ...
    setProgress(60);
    setStatus('Converting Excel → PDF...');
    ...
    const base = (file?.name?.replace(/\.(xlsx|xls)$/i,'') || 'sheet');
